        ...
        if(o.nodes.length > 1)
        {
          ...
          items.push({
            ...
            handler: function () {
              ...
              var seen = {}, found, changed = [], multi = false, f = function(node) { // called when node added successfully
              ...
              if(!found || multi) // gotta make a new node, tricky
              {
                Ext.Msg.show({
                  title:'Important',
...
//           height: 35,
  ...
  deleteNode: function(n) {
    ...
    Ext.Msg.confirm('Warning', 'Are you sure you want to delete '+str1+'? '+str2+' edges will be deleted!', function(b) {
      if(b == 'yes')
      {
        if(!this.viewInfo) this.fireEvent('removenode', n, function() {
          for(var i = 0; i < n.length; i++)
  ...
  deleteEdge: function(e) {
    Ext.Msg.confirm('Warning', 'Are you sure you want to delete this edge?', function(b) {
      if(b == 'yes')
      {
        if(!this.viewInfo) this.fireEvent('removeedge', e, function() {
  ...
  revertView: function(force) {
    if(this.mainView)
    {
      ...
      if(force !== 1 && this.hasUnsavedChanges() && this.hasListener('saveallchanges'))
        Ext.Msg.confirm('Warning!', 'If you proceed by clicking "Yes", <u>any unsaved changes in the '+(this.viewInfo? 'view "'+this.viewInfo.name+'"' : 'original view')+' will be discarded</u>. Are you sure you would like to proceed?<br><br>Tip: To save unsaved changes first, please click "No" below, then ' + (this.viewInfo? 'use the View Menu\'s "Save Current View" to save.' : 'either enter superuser mode or press save button to save.'), function(b) {
          if(b == 'yes') f()
        }, this);
      else f();
    }
    ...
    var bf = function(b, e, noClose) {
      ...
      for(var i = 0; i < allchanged.length; i++)
      {
        ...
        else
        {
          ...
          {
            if(!this.viewInfo) this.extCanvas.fireEvent('updatenode', isMulti? conf[i] : conf, p, f);
          }
...
Ext.canvasXpress.edgeDialog = Ext.extend(Ext.Window, {
  ...
  constructor: function(config, dir, extCanvas, config1) {
    ...
    var bbf = function(b, e, noClose) {
      ...
      {
        if(!this.viewInfo) this.extCanvas.fireEvent('updateedge', this.edgeInfo, p, f);
      }
