...
public class ShippedOrdersActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if(conn != null){
			...
			title.setText(R.string.shipped_orders);
	...
	protected void onResume() {
		if(api != null){
			new GetShippedOrdersTask().execute(api);
		}
		super.onResume();
	}
	
	private class GetShippedOrdersTask extends AsyncTask<Gas, Void, Order[]>{

		@Override
		protected Order[] doInBackground(Gas... params) {
			Gas gas = params[0];
			
			return gas.orderOperations().getRespShippedOrders();
		}
		
		@Override
		protected void onPostExecute(Order[] result) {
			if(result != null && result.length > 0){
				adapter = new CustomAdapter(ShippedOrdersActivity.this, R.layout.order_available_item, R.id.orderName, result);
	...
	private class CustomAdapter extends ArrayAdapter<Order>{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			//Visualizzazione data di consegna
			orderDeliveryLabel = (TextView) row.findViewById(R.id.delivery_date_label);
			orderDeliveryLabel.setVisibility(View.VISIBLE);
			...
			orderDetails.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					Intent intent = new Intent(getApplicationContext(), ShipOrderPurchasesActivity.class);
