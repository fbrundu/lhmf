...
public class SupplierActiveOrdersActivity extends Activity {
	...
	protected void onResume() {
		if(api != null){
			new GetSupplierActiveOrdersTask().execute(api);
		}
		super.onResume();
	}
	...
	private class GetSupplierActiveOrdersTask extends AsyncTask<Gas, Void, Order[]>{

		@Override
		protected Order[] doInBackground(Gas... params) {
			Gas gas = params[0];
			return gas.orderOperations().getSupplierActiveOrders();
		}
		...
		protected void onPostExecute(Order[] result) {
			if(result != null && result.length > 0){
				...
				adapter = new CustomAdapter(SupplierActiveOrdersActivity.this, R.layout.order_available_item, R.id.orderName, result);
	...
	private class CustomAdapter extends ArrayAdapter<Order>{
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			orderSupplier.setVisibility(View.GONE);
			supplierLabel = (TextView) row.findViewById(R.id.supplierLabel);
