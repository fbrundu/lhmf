...
public class ActiveOrdersActivity extends Activity {
	...
	protected void onResume() {
		if(api != null){
			new GetActiveOrdersTask().execute(api);
		}
		super.onResume();
	}
	...
	private class GetActiveOrdersTask extends AsyncTask<Gas, Void, Order[]>{

		@Override
		protected Order[] doInBackground(Gas... params) {
			Gas gas = params[0];
			return gas.orderOperations().getRespActiveOrders();
		}
		...
		protected void onPostExecute(Order[] result) {
			if(result != null && result.length > 0){
				...
				adapter = new CustomAdapter(ActiveOrdersActivity.this, R.layout.order_available_item, R.id.orderName, result);
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			orderDetails.setOnClickListener(new View.OnClickListener() {
				...
				public void onClick(View v) {
					//TODO start order details activity, similar to new_purchase and edit_purchase
